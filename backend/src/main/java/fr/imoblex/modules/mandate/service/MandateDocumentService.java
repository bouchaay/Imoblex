...
public class MandateDocumentService {
    ...
    /**
     * Logo de l'agence positionné en absolu dans le coin supérieur droit du header,
     * au-dessus de "ENTRE LES SOUSSIGNÉS" — n'affecte pas le flux du document.
     */
    private void tryAddHeaderLogo(PdfWriter writer, AgencySettings ag) {
        try {
            if (ag.getLogoPath() == null || ag.getLogoPath().isBlank()) return;
            File logoFile = resolveSignatureFile(ag.getLogoPath());
            if (logoFile == null || !logoFile.exists()) return;

            Image logo = Image.getInstance(logoFile.getAbsolutePath());
            logo.scaleToFit(80f, 28f);

            float pageW = PageSize.A4.getWidth();
            float pageH = PageSize.A4.getHeight();
            // Coin supérieur droit : marge droite 28pt, marge haute 20pt
            float x = pageW - 28f - logo.getScaledWidth();
            float y = pageH - 20f - logo.getScaledHeight();
            logo.setAbsolutePosition(x, y);
            writer.getDirectContent().addImage(logo);
        } catch (Exception e) {
            log.warn("Header logo introuvable: {}", e.getMessage());
        }
    }
